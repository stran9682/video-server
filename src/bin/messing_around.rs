...
fn main() {
    ...
    let authorized_entry = alfie_entry.into_authorised_entry(&communal_cap, &alfie_secret);
    ...
    read_cap.delegate(&alfie_secret, Area::new(Some(alfie_id.clone()), path!("/ideas"), TimeRange::Open(0.into())), betty_id.clone());

    // nice try betty!
    let restricted_access = read_cap.includes_area(&Area::new(Some(alfie_id), path!("/restricted"), TimeRange::Open(0.into())));
