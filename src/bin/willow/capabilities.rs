...
fn main() {
    ...
    let communal_namespace_id = NamespaceId::from_bytes(&[17; NAMESPACE_ID_WIDTH]);
    ...
    let communal_authored = entry_communal.into_authorised_entry(&communal_cap, &alfie_secret);

    assert!(communal_authored.is_ok());
    println!("Entry for communal namespace was authorized");
